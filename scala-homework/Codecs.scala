...
trait EncoderInstances:
  ...
  given stringEncoder: Encoder[String] =
    Encoder.fromFunction(string => Json.Str(string))// TODO Implement the `Encoder[String]` given instance
...
trait DecoderInstances:
  ...
  given StrDecoder: Decoder[String] =
    Decoder.fromPartialFunction{ case Json.Str(s: String) => s.toString}
  /** A decoder for `String` values */
  // TODO Define a given instance of type `Decoder[String]`

  given BoolDecoder: Decoder[Boolean] =
    Decoder.fromPartialFunction{ case Json.Bool(k: Boolean) => k}
  ...
  given ListDecoder[A](using decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromFunction {
      case Json.Arr(ar1) => ar1.foldLeft(Some(List.empty[A]): Option[List[A]])
          {
            (list1, ar1) => list1 match
              case Some(list2) => decoder.decode(ar1) match
                case Some(list3) => Some(list2 :+ list3)
                case None => None
              case None => None
              }
        case _ => None
  ...
  def field[A](name: String)(using decoder: Decoder[A]): Decoder[A] =
    Decoder.fromFunction{
      case Json.Obj(ob1) => decoder.decode(ob1.get(name).get)
      case _ => None
    }
...
trait PersonCodecs:
  ...
  given Decoder[Person] = Decoder
    .field[String]("name")
    .zip(Decoder.field[Int](name = "age"))
...
trait ContactsCodecs:
  ...
  given Encoder[Contacts] = ObjectEncoder.field[List[Person]]("people").transform[Contacts](cts => cts.people)
  // ... then implement the decoder
  given Decoder[Contacts] = Decoder.field[List[Person]]("people").transform[Contacts](people => Contacts(people))
