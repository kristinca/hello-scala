...
final class Wikigraph(client: Wikipedia):
  ...
  def namedLinks(of: ArticleId): WikiResult[Set[String]] =
    client.linksFrom(of).flatMap{
      id => WikiResult.traverse[ArticleId, String](id.toSeq)(client.nameOfArticle).map(i => i.toSet)
    }
  ...
  def breadthFirstSearch(start: ArticleId, target: ArticleId, maxDepth: Int): WikiResult[Option[Int]] =
    ...
    def iter(visited: Set[ArticleId], q: Queue[(Int, ArticleId)]): WikiResult[Option[Int]] ={
      if q.isEmpty then WikiResult.successful(None)
        ...
        val ((cedepth, ceArt), leftqueue) = q.dequeue
        if cedepth >= maxDepth then WikiResult.successful(None)
        else {
          client.linksFrom(ceArt).flatMap(ArtId=> {
            if ArtId.contains(target) then WikiResult.successful(Some(cedepth))
            else
              iter(visited+ceArt, leftqueue.enqueueAll(ArtId.filter(aid=>
              !visited.contains(aid)).map(aid => (cedepth+1, aid))) )}
          ).fallbackTo(iter(visited+ceArt,leftqueue))}}
      }
  ...
  def distanceMatrix(titles: List[String], maxDepth: Int = 50): WikiResult[Seq[(String, String, Option[Int])]] =
    ...
    WikiResult.traverse[(String, String),(String,String,Option[Int])]
      (TousPairs){
        (titleOne, titleTwo)=>
          client.searchId(titleOne)
            .zip(client.searchId(titleTwo))
            .flatMap{
              (idOne, idTwo)=>
                breadthFirstSearch(idOne, idTwo, maxDepth)
                  .flatMap{
                    howfar =>
                      WikiResult.successful(titleOne, titleTwo, howfar)
                  }}
            }
end Wikigraph
