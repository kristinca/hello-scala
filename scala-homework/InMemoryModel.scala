...
//    found
  ...
  def tags: Tags =
    val vsiTags = idStore.values.flatMap(_.tags).toList.distinct
    Tags(vsiTags)
//    Tags(List.empty)

  def tasks(tag: Tag): Tasks =
    def leTag (task: Task, tag: Tag): Boolean =
      task.tags.contains(tag)

    val leTaskpourCeTag = idStore.filter((_, task) => leTag(task, tag))
    Tasks(leTaskpourCeTag)
//    Tasks(idStore)
